...
const App = {
  ...
  cacheDOM() {
    this.dom = {
      ...
      fcFrontDrug: document.getElementById('fcFrontDrug'),
...
App.renderFlashcard = function() {
  ...
  if (queue.length === 0) {
    ...
    this.dom.fcFrontDrug.textContent = '';
  ...
  if (currentIndex >= queue.length) {
    ...
    this.dom.fcFrontDrug.textContent = `Правильно отмечено: ${this.state.flashcard.queue.filter(r => this.state.learnedIds.includes(r.id)).length} / ${queue.length}`;
  ...
  this.dom.fcFrontDrug.textContent = recipe.drug;
